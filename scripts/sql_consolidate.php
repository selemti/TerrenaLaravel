...
function registerObject(&$seen, &$dups, $kind, $name)
{
    $k = keyNorm($name);
...
function extractBlocks($sql)
{
    ...
    while ($i < $len) {
        $ch = $sql[$i];
        if (! $inDo && stripos(substr($sql, $i, 3), 'DO ') === 0 && preg_match('/DO\s*\$\$/Ai', substr($sql, $i, 6))) {
            $inDo = true;
        }
        $buf .= $ch;
...
function detectObject($blk)
{
    ...
    // Schemas
    if (preg_match('/create\s+schema\s+(if\s+not\s+exists\s+)?([a-z0-9_\.]+)/i', $blk, $m)) {
        return ['schema', $m[2]];
    }
    if (preg_match('/create\s+type\s+(if\s+not\s+exists\s+)?([a-z0-9_\.]+)/i', $blk, $m)) {
        return ['type', $m[2]];
    }
    if (preg_match('/create\s+(?:temporary\s+|unlogged\s+)?table\s+(if\s+not\s+exists\s+)?([a-z0-9_\.\"]+)/i', $blk, $m)) {
        return ['table', trim($m[2], '"')];
    }
    if (preg_match('/create\s+(unique\s+)?index\s+(?:concurrently\s+)?(if\s+not\s+exists\s+)?([a-z0-9_\.\"]+)\s+on\s+([a-z0-9_\.\"]+)/i', $blk, $m)) {
        return ['index', trim($m[3], '"').'::'.trim($m[4], '"')];
    }
    if (preg_match('/alter\s+table\s+([a-z0-9_\.\"]+)\s+add\s+constraint\s+([a-z0-9_\.\"]+)/i', $blk, $m)) {
        return ['constraint', trim($m[1], '"').'::'.trim($m[2], '"')];
    }
    if (preg_match('/create\s+view\s+([a-z0-9_\.\"]+)\s+as/i', $blk, $m)) {
        return ['view', trim($m[1], '"')];
    }
    if (preg_match('/create\s+function\s+([a-z0-9_\.\"]+)\s*\(/i', $blk, $m)) {
        return ['function', trim($m[1], '"')];
    }
    if (preg_match('/create\s+trigger\s+([a-z0-9_\.\"]+)\s+.*?\s+on\s+([a-z0-9_\.\"]+)/is', $blk, $m)) {
...
function consolidate($baseFile, $otherFiles, $out, &$seen, &$dups)
{
    ...
    foreach ($order as $f) {
        ...
        foreach ($blocks as $blk) {
            ...
            if ($kind) {
                if (! registerObject($seen, $dups, $kind, $obj)) {
                    continue;
                }
            }
