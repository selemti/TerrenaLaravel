...
$bd = __DIR__.'/../BD';
if (! is_dir($bd)) {
    fwrite(STDERR, "BD folder not found\n");
    exit(1);
}
$ts = date('Ymd-His');
...
if (! $base) {
    fwrite(STDERR, "Base file not found\n");
    exit(2);
}
$all = glob($bd.'/*.sql');
$others = array_values(array_filter($all, function ($f) use ($base) {
    return realpath($f) !== realpath($base);
}));
sort($others, SORT_NATURAL | SORT_FLAG_CASE);

$seen = ['schema' => [], 'type' => [], 'table' => [], 'constraint' => [], 'index' => [], 'view' => [], 'function' => [], 'trigger' => []];
$dups = [];
function reg(&$seen, &$dups, $kind, $name)
{
    $k = keyNorm($name);
    if (! $kind) {
        return true;
...
function pushBlock(&$stages, $kind, $blk)
{
...
function feedFile($file, &$stages, &$seen, &$dups)
{
    ...
    foreach ($blocks as $blk) {
        if ($blk === '') {
            continue;
        } [$kind, $obj] = detectObject($blk);
        if ($kind && ! reg($seen, $dups, $kind, $obj)) {
            continue;
        } pushBlock($stages, $kind ?: 'misc', $blk);
    }
}
...
foreach ($dups as $k => $arr) {
    $rm[] = "- $k: ".count($arr);
    if ($arr) {
        $rm[] = '  - '.implode(', ', array_slice(array_unique($arr),0,10));
    }
}
