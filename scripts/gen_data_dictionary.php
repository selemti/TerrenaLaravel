...
$byTable = [];
foreach ($tables as $t) {
    $byTable[$t->table_schema][$t->table_name] = ['type' => $t->table_type];
}
foreach ($columns as $c) {
    $byTable[$c->table_schema][$c->table_name]['cols'][] = $c;
}
foreach ($pks as $r) {
    $byTable[$r->table_schema][$r->table_name]['pk'][] = $r->column_name;
}
foreach ($fks as $r) {
    $byTable[$r->table_schema][$r->table_name]['fk'][] = $r;
}
foreach ($uniques as $u) {
    $byTable[$u->table_schema][$u->table_name]['uq'][] = $u;
}
foreach ($checks as $ck) {
    $byTable[$ck->table_schema][$ck->table_name]['ck'][] = $ck;
}
$idxMap = [];
foreach ($indexes as $ix) {
    $idxMap[$ix->schema][$ix->table][] = $ix;
}
$approxMap = [];
foreach ($approx as $r) {
    $approxMap[$r->schema][$r->table] = (int) $r->approx_rows;
}
$seqMap = [];
foreach ($sequences as $s) {
    if ($s->sequence) {
        $seqMap[$s->schema][$s->table][$s->column] = $s->sequence;
    }
}
...
function hasIndexForFk($schema, $table, $col, $idxMap)
{
    ...
    foreach ($idxMap[$schema][$table] as $ix) {
        ...
        if (isset($cols[0]) && $cols[0] === $col) {
            return true;
        } // starts with FK column
    }
    ...
    foreach ($byTable[$schema] as $table => $info) {
        ...
        foreach ($info['cols'] ?? [] as $c) {
            ...
            if ($c->numeric_precision) {
                $type .= '('.$c->numeric_precision.','.(($c->numeric_scale !== null) ? $c->numeric_scale : 0).')';
            }
        ...
        fwrite($f, "\n### Únicas\n");
        if (! empty($info['uq'])) {
            foreach ($info['uq'] as $u) {
                fwrite($f, "- {$u->constraint_name} — [{$u->columns}]\n");
            }
        } else {
            fwrite($f, "- N/A\n");
        }
        ...
        fwrite($f, "\n### Checks\n");
        if (! empty($info['ck'])) {
            foreach ($info['ck'] as $ck) {
                fwrite($f, "- {$ck->constraint_name}: {$ck->definition}\n");
            }
        } else {
            fwrite($f, "- N/A\n");
        }
        ...
        if (! empty($seqMap[$schema][$table])) {
            foreach ($seqMap[$schema][$table] as $col => $seq) {
                fwrite($f, "- $col ? $seq\n");
            }
        } else {
            fwrite($f, "- N/A\n");
        ...
        foreach (($info['cols'] ?? []) as $c) {
            if (strtolower($c->column_name) !== 'id' && $c->data_type === 'double precision') {
                $flags['monetario_no_uniforme'] = true;
            }
        }
...
foreach ($fks as $fk) {
    ...
    if (! in_array($child, $known) && ! in_array($parent, $known)) {
        continue;
    }
    $label = $fk->column_name.' -> '.$fk->foreign_column_name;
