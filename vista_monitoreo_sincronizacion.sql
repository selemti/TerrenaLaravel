-- Vista de monitoreo para sincronización Floreant-Laravel
...
SELECT
    ...
    aud.quie as audit_quien,
    aud.que as audit_resultado,
    aud.ts as audit_timestamp,
...
LEFT JOIN (
    SELECT max(id) as max_id, quien, que, ts, payload
    FROM selemti.auditoria
    WHERE que IN ('SESION_CREADA', 'SESION_CERRADA', 'ERROR_SESION', 'ERROR_CIERRE', 'SIN_TERMINAL')
    GROUP BY quien, que, ts, payload
) aud ON aud.quien = dah.a_user AND aud.ts::timestamp = (
    SELECT MAX(ts) FROM selemti.auditoria a
    WHERE a.quien = dah.a_user AND a.payload->>'dah_id' = dah.id::text
)
...
WHERE aud.quie IN ('ERROR_SESION', 'ERROR_CIERRE', 'ERROR_TERMINAL', 'ERROR_CRITICO', 'SIN_TERMINAL')
  AND aud.ts >= CURRENT_DATE - INTERVAL '1 day'
ORDER BY aud.ts DESC;

-- 4. Verificación de las vistas creadas
SELECT 'Vistas de monitoreo creadas correctamente' as resultado;
