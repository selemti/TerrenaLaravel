...
$tickets = DB::connection('pos_pg')->table('public.ticket')->orderBy('id', 'desc')->limit(50)->get();
foreach ($tickets as $t) {
    $item = DB::connection('pos_pg')->table('public.ticket_item')->where('ticket_id', $t->id)->first();
    if ($item) {
        echo "VALID TICKET FOUND: ID={$t->id} | ITEM_ID={$item->item_id} | NAME={$item->item_name}\n";
        break;
    }
}
