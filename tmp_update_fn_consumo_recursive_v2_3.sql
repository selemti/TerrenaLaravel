-- =============================================================================
-- REFACTORIZACIÓN FINAL: fn_expandir_consumo_ticket (v2.3)
-- SOPORTE: Recursividad Logística + Stock-Aware + Modificadores (Branch Mapping)
-- =============================================================================
...
BEGIN
    ...
    SELECT DISTINCT
        ti.ticket_id,
        ti.id,
        COALESCE(s.id, 0), -- Resolvemos sucursal_id numérico desde la clave del POS
        t.terminal_id,
        'PENDIENTE',
        now()
    FROM public.ticket_item ti
    JOIN public.ticket t ON t.id = ti.ticket_id
    LEFT JOIN selemti.cat_sucursales s ON s.clave = t.branch_key OR s.clave = 'SUC01' -- Fallback para pruebas
    ...
    VALUES (_ticket_id, 'EXPAND_RECURSIVE_2026_FINAL_V2_3', jsonb_build_object('ts', now()));
END;
$function$;
