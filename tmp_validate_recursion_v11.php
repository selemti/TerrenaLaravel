...
try {
    // 0. Apply the SQL function v2.2 first inside the script for safety
    $sql = file_get_contents(__DIR__ . '/tmp_update_fn_consumo_recursive_v2_2.sql');
    DB::connection('pgsql')->unprepared($sql);
    echo "SUCCESS: Recursive Function v2.2 Applied.\n";
    ...
    // Create a Mock Ticket (using Floreant schema)
    $ticketId = 12 * (time() % 50000); // More room for safety
    DB::connection('pos_pg')->unprepared("DELETE FROM public.ticket_item WHERE ticket_id = $ticketId");
    DB::connection('pos_pg')->unprepared("DELETE FROM public.ticket WHERE id = $ticketId");

    DB::connection('pos_pg')->table('public.ticket')->insert([
        'id' => $ticketId, 'create_date' => now(), 'terminal_id' => 689, 'branch_key' => 'SUR', 'total_price' => 100
    ]);
    
    // We don't specify ID for ticket_item to let DB handle it
    DB::connection('pos_pg')->table('public.ticket_item')->insert([
        'ticket_id' => $ticketId, 'item_id' => 99999, 'item_name' => 'Test Base', 'item_quantity' => 1, 'item_price' => 100, 'tax_amount' => 0, 'item_count' => 1
    ]);
