...
export async function abrirWizard(ev){
  ...
  try{
    ...
    if (sesion){
      ...
      const pre = await GET(`${BASE}/api/legacy/sprecorte/preflight?sesion_id=${sesionId}`);
			console.log('Preflight response:', pre);
			if (pre?.bloqueo){
    ...
    try{
      const st = await GET_SOFT(`${BASE}/api/precortes/${state.precorteId}/status`);
...
async function guardarPrecorte(){
  ...
  try{
    ...
    try { await POST_FORM(`${BASE}/api/caja/precorte_status.php?id=${state.precorteId}`, { sesion_estatus:'LISTO_PARA_CORTE', precorte_estatus:'ENVIADO' }); } catch(_){}
    ...
    if (window.bootstrap?.Modal){
      ...
      el.addEventListener('keydown', (ev)=>{
        ...
        if (ev.key === 'Enter')  { ev.preventDefault(); btnOk.click(); }
			...
			console.log('Showing modal, preflight:', pre);
      m.show();
      return;
    }
