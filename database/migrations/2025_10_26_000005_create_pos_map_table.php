...
return new class extends Migration
{
    ...
    {
        if ($this->tableExists('pos_map')) {
            return;
        }

        Schema::connection('pgsql')->create('selemti.pos_map', function (Blueprint $table) {
            ...
            $table->string('pos_code')->index('idx_pos_map_code')->comment('Code or identifier from the POS for this product/modifier.');
            ...
            $table->boolean('is_modifier')->default(false)->index('idx_pos_map_modifier')->comment('If true, this record represents a POS modifier.');
            ...
            $table->boolean('is_active')->default(true)->index('idx_pos_map_active')->comment('Allows deactivating a mapping without deleting it.');
            $table->timestamps();
