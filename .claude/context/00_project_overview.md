# ⚠️ INVARIANTE CRÍTICO — DB_SCHEMA en phpunit.xml

`DB_SCHEMA` en `phpunit.xml` debe ser SIEMPRE solo `selemti`. Nunca agregar `public`.

Si incluye `public`, `RefreshDatabase` → `migrate:fresh` → `dropAllTables()` borra las 108 tablas de FloreantPOS (`ticket`, `terminal`, `cash_drawer`…) en cada test run. Ocurrió el **2026-05-13**, requirió restore desde producción. El test `tests/Unit/GuardRailsTest` detecta esto automáticamente.

El schema `public` es **READ ONLY** — pertenece a FloreantPOS en producción.
