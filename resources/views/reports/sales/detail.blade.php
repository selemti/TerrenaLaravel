        ...
        <div class="card-body">
            <form method="GET" action="{{ route('reports.sales.detail') }}" class="row g-3 align-items-end">
                ...
                <div class="col-md-2 d-flex gap-2">
                    ...
                    <a href="{{ route('reports.sales.detail') }}" class="btn btn-outline-secondary flex-fill"><i class="fa-solid fa-rotate-left me-1"></i> Limpiar</a>
                </div>
                    ...
                    <tbody>
                        @forelse($rows as $r)
                            <tr>
                                ...
                                <td class="text-end">${{ number_format((float)($r->unit_price ?? 0), 2) }}</td>
                                <td class="text-end">${{ number_format((float)($r->line_discount ?? 0), 2) }}</td>
                                <td class="text-end">${{ number_format((float)($r->line_neto ?? 0), 2) }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="9" class="text-center text-muted py-4">Sin datos</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
