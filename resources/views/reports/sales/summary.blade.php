        ...
        <div class="card-body">
            <form method="GET" action="{{ route('reports.sales.summary') }}" class="row g-3 align-items-end">
                ...
                <div class="col-md-2 d-flex gap-2">
                    ...
                    <a href="{{ route('reports.sales.summary') }}" class="btn btn-outline-secondary flex-fill"><i class="fa-solid fa-rotate-left me-1"></i> Limpiar</a>
                </div>
                    ...
                    <tbody>
                        @forelse($rows as $r)
                            <tr>
                                ...
                                <td>{{ $r->branch_key ?? '—' }}</td>
                                <td class="text-end">{{ number_format((int)($r->tickets ?? 0)) }}</td>
                                <td class="text-end">${{ number_format((float)($r->bruto ?? 0), 2) }}</td>
                                <td class="text-end">${{ number_format((float)($r->descuento ?? 0), 2) }}</td>
                                <td class="text-end">${{ number_format((float)($r->neto ?? 0), 2) }}</td>
                                <td class="text-end">${{ number_format((float)($r->propina ?? 0), 2) }}</td>
                                <td class="text-end">${{ number_format((float)($r->cargo_servicio ?? 0), 2) }}</td>
                            </tr>
                        @empty
