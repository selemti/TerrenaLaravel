...
class AuthServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        Gate::define('cashfund.view', function ($user) use ($safe) {
            if (! $user) {
                return false;
            }

            if ($safe(fn () => method_exists($user, 'hasAnyRole') && $user->hasAnyRole(['Super Admin', 'Ops Manager', 'cashier']))) {
                return true;
            }

            return $safe(fn () => method_exists($user, 'hasPermissionTo') && $user->hasPermissionTo('cashfund.view'));
        });

        Gate::define('cashfund.manage', function ($user) use ($safe) {
            if (! $user) {
                return false;
            }

            if ($safe(fn () => method_exists($user, 'hasAnyRole') && $user->hasAnyRole(['Super Admin', 'Ops Manager']))) {
                return true;
            }

            return $safe(fn () => method_exists($user, 'hasPermissionTo') && $user->hasPermissionTo('cashfund.manage'));
        });
