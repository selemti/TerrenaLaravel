-- Trigger corregido para sincronización Floreant-Laravel
-- Corrige el problema con jsonb_build_object que causa ERROR_CRITICO
...
BEGIN
    ...
    -- Construir información de debugging como TEXT para evitar problemas con jsonb
    v_debug_info := format('DAH_ID:%s, OPER:%s, USER:%s, TIME:%s',
                          NEW.id,
                          COALESCE(NEW.operation, 'NULL'),
                          NEW.a_user,
                          NEW.time);
    ...
    BEGIN
        ...
        IF v_terminal_id IS NULL THEN
            ...
            VALUES(NEW.a_user, v_operation_result,
                   jsonb_build_object('debug', v_debug_info, 'error', v_error_message));
            RETURN NEW;
        END IF;
    ...
    WHEN OTHERS THEN
        ...
        BEGIN
            ...
            VALUES(NEW.a_user, 'ERROR_CRITICO',
                   jsonb_build_object('debug', v_debug_info, 'critical_error', SQLERRM));
        EXCEPTION
            WHEN OTHERS THEN
...
FOR EACH ROW EXECUTE PROCEDURE selemti.fn_dah_corregido();
