<?php
/**
 * ANÁLISIS COMPLETO DE DISCREPANCIAS EN VENTAS - CORREGIDO
 * Agosto, Septiembre y Octubre 2025
 * Usa los valores correctos de la tabla ticket
 */
...
echo "║          INCLUYE TODOS LOS MÉTODOS DE PAGO (Versión Corregida)              ║\n";
echo "╚══════════════════════════════════════════════════════════════════════════════╝\n\n";
...
function analyzeMonth($monthName, $startDate, $endDate) {
    ...
    $dailySummary = DB::connection('pgsql')->select("
        WITH daily_data AS (
            SELECT 
                ...
                -- TOTALES DESDE LA TABLA TICKET (no desde ticket_item)
                COALESCE(SUM(t.sub_total), 0) AS total_items_bruto,
                COALESCE(SUM(t.total_discount), 0) AS total_descuentos,
                COALESCE(SUM(CASE WHEN t.total_discount >= t.sub_total AND t.total_discount > 0 THEN t.total_discount ELSE 0 END), 0) AS descuentos_100_pct,
                COALESCE(SUM(t.total_price), 0) AS total_neto_tickets,
                ...
                COALESCE(SUM(CASE WHEN tr.transaction_type IN ('REFUND', 'VOID') THEN ABS(tr.amount) ELSE 0 END), 0) AS total_refunds,
        ...
        SELECT 
            ...
            total_refunds,
            total_pagos,
            (total_pagos - total_refunds) AS pagos_netos,
            (total_neto_tickets - (total_pagos - total_refunds)) AS diferencia
        FROM daily_data
    ...
    foreach ($dailySummary as $day) {
        ...
        echo sprintf(
            ...
            "   💵 Bruto: $%s | Desc: $%s (100%%: $%s) | NETO: $%s\n" .
            "   💰 PAGOS: Cash $%s | Credit $%s | Debit $%s | Transfer $%s | Refund $%s\n" .
            "   📈 Pagos Netos: $%s | ⚠️  DIFERENCIA: $%s\n\n",
            $day->fecha,
            $day->total_tickets,
            $day->tickets_pagados,
            ...
            number_format($day->total_neto_tickets, 2),
        ...
        $totalVentaNeta += $day->total_neto_tickets;
        $totalPagosNetos += $day->pagos_netos;
    }
    
    echo "\n" . str_repeat("-", 80) . "\n";
    echo sprintf(
        "🎯 TOTAL DEL MES ({$dias} días):\n" .
        "   Venta Neta (tickets): $%s\n" .
    ...
    $problematicTickets = DB::connection('pgsql')->select("
        SELECT 
            ...
            CASE 
                ...
                WHEN t.total_discount >= t.sub_total AND t.total_discount > 0 THEN 'DESC_100_PCT'
                WHEN ABS(t.total_price - COALESCE(SUM(CASE WHEN tr.transaction_type NOT IN ('REFUND', 'VOID') THEN tr.amount ELSE 0 END), 0)) > 0.50 
                THEN 'PAGO_DIFERENTE'
                ELSE 'OTRO'
        ...
        LEFT JOIN public.transactions tr ON t.id = tr.ticket_id
        WHERE t.folio_date BETWEEN ? AND ?
        GROUP BY t.id, t.folio_date, t.paid, t.voided, t.sub_total, t.total_discount, t.total_price
        HAVING 
            (t.paid = FALSE AND t.voided = FALSE)
            OR t.voided = TRUE
            OR (t.total_discount >= t.sub_total AND t.total_discount > 0)
            OR ABS(t.total_price - COALESCE(SUM(CASE WHEN tr.transaction_type NOT IN ('REFUND', 'VOID') THEN tr.amount ELSE 0 END), 0)) > 0.50
    ...
    foreach ($problematicTickets as $ticket) {
        ...
        $problemTotals[$ticket->tipo_problema] += $ticket->total_price;
        
        if ($problemCounts[$ticket->tipo_problema] <= 10) {  // Mostrar solo los primeros 10 de cada tipo
            echo sprintf(
                "🎫 Ticket #%d [%s] - %s\n" .
                "   Bruto: $%s | Desc: $%s | Neto: $%s | Pagado: $%s (C:$%s Cr:$%s D:$%s T:$%s)\n",
                $ticket->id,
                $ticket->fecha,
                $ticket->tipo_problema,
                number_format($ticket->sub_total, 2),
                number_format($ticket->total_discount, 2),
...
echo "\n✅ Análisis completado - archivo: analisis_ventas_FINAL.txt\n\n";
