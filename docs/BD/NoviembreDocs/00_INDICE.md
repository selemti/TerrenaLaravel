...
**Ubicación**: `docs/docs/BD/NoviembreDocsDocs/`
...
1. Crear backup completo:
   ...
   pg_dump -h localhost -p 5433 -U postgres -d pos -F c \
     -f docs/docs/BD/NoviembreDocsDocs/backup_pre_limpieza_$(date +%Y%m%d_%H%M%S).dump
   ```
2. Ejecutar dry-run:
   ```bash
   psql -h localhost -p 5433 -U postgres -d pos \
     -f docs/docs/BD/NoviembreDocsDocs/drop_legacy_safe.sql
   ```
